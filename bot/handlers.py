...
def register_handlers(
    ...
    async def admin(message: Message):
        ...
        await message.answer(
            "Постоянное меню администратора включено.",
            reply_markup=kb.admin_reply_menu(),
        )

    # Постоянное меню Telegram. Оно остаётся доступным даже после
    # появления большого количества сообщений в чате.
    @router.message(F.text == "📱 WireGuard")
    async def reply_user_wireguard(message: Message):
        row = app.db.get_user(message.from_user.id)
        if not _approved(row):
            await message.answer("Доступ к WireGuard пока не разрешён.")
            return
        await message.answer(
    ...
    async def reply_admin_user_mode(message: Message):
        if message.from_user.id not in app.settings.admin_ids:
            return
        row = app.db.get_user(message.from_user.id)
        if not _approved(row):
            await message.answer("Администратор не имеет одобренной пользовательской записи в базе.")
            return
        await message.answer(
    ...
    async def reply_admin_mode(message: Message):
        if message.from_user.id not in app.settings.admin_ids:
            return
        await message.answer(
            "🔐 Администрирование",
